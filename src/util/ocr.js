...
export function getDfilesText(file, callback) {
	Jimp.read(file)
		.then((image) => {
			image.scan(0, 0, image.bitmap.width, image.bitmap.height, function(x, y, idx) {
				...
				if (y < 5 || x < 5) {
					//Remove header text & side pixels
					image.setPixelColor(white, x, y);
				} else if (rgbColor['r'] > 150 && rgbColor['g'] > 150 && rgbColor['b'] > 150) {
					...
					getAngleToStraightTheImage(image, 0, 1, image.bitmap.height, 0, function(angleToRotate) {
						...
						changeAllPresentPixelsToBlack(orgImg, function(orgImg) {
							...
							removeSpikes(orgImg, 1, function(orgImg) {
								orgImg.getBuffer(Jimp.MIME_JPEG, function(err, data) {
								...
								orgImg.write('indextest.jpg', function() {
									//Print current image
								});
								//})
							});
...
function getAngleToStraightTheImage(image, maxYPixel, round, OrgHeight, angle, callback) {
	for (var y = 0; y < image.bitmap.height; y++) {
		for (var x = 0; x < image.bitmap.width; x++) {
			var color = image.getPixelColor(x, y);
			var rgbColor = Jimp.intToRGBA(color);
			if (color != white && color != black && color != 0 && color != 4294967045) {
				//Color pixel found
	...
	Jimp.read(file)
		.then((image) => {
			image.scan(0, 0, image.bitmap.width, image.bitmap.height, function(x, y, idx) {
				...
				if (index != white && y < 10) {
					//Collect all colors represented in the top 10 pixels of the image
					borderColorsTop[index] = true;
				}

				if (index != white && image.bitmap.height - y < 10) {
					//Collect all colors represented in the bottom 10 pixels of the image
				...
				if (x == image.bitmap.width - 1 && y == image.bitmap.height - 1) {
					...
					for (var k in borderColorsTop) {
						//Ban all colors that are represented in the top AND bottom 10 pixels of the image (So if the text is moved to the top we do not ban the text color!)
					...
					for (var i in colorBuffer) {
						if (colorBuffer[i] < 100) {
							//Remove all colors witch are not very present in the img (less then 100 pixels of this color)
							bandColors[i] = true;
						}
					}
					...
					image.scan(0, 0, image.bitmap.width, image.bitmap.height, function(x, y, idx) {
						var color = image.getPixelColor(x, y);
						var rgbColor = Jimp.intToRGBA(color);
						if (!isColorVisableWellOnWhite(rgbColor)) {
							//remove colors that are barley seen
							image.setPixelColor(white, x, y);
						} else {
						...
						if (x == image.bitmap.width - 1 && y == image.bitmap.height - 1) {
							fillGaps(image, 3, function(image) {
								thinOut(image, 2, function(image) {
									changeAllPresentPixelsToBlack(image, function(image) {
										// removeSpikes(image, function(image) {
						...
						for (var y = 0; y < binArray.length; y++) {
							for (var x = 0; x < binArray[y].length; x++) {
								if (binArray[y][x] == index) {
									...
									imgObj[index]['lx'] =
										imgObj[index]['lx'] && imgObj[index]['lx'] < x ? imgObj[index]['lx'] : x;
									imgObj[index]['ly'] =
										imgObj[index]['ly'] && imgObj[index]['ly'] < y ? imgObj[index]['ly'] : y;
...
function contrast(rgb1, rgb2) {
	return (luminanace(rgb1['r'], rgb1['g'], rgb1['b']) + 0.05) / (luminanace(rgb2['r'], rgb2['g'], rgb2['b']) + 0.05);
}

//Returns true if the given color is "readable" for humans on white background
function isColorVisableWellOnWhite(rgbColor) {
	if (
		contrast(Jimp.intToRGBA(white), rgbColor) < 2 &&
		rgbColor['r'] > 150 &&
		rgbColor['g'] > 150 &&
		rgbColor['b'] > 150
	) {
